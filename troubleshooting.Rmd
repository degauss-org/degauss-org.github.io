...
In a DeGAUSS command, the `-v` argument is tells Docker to bind the current directory to the `/tmp` directory inside a container so that it can read the input file and write the output file back to the current working directory on the host platform.
